...
"""
    plot_ll_grid(point::Vector, noise_vals::Vector, M_vals::Vector, lower_bound::Float64, path_to_read::String, w0::Float64, m::Float64, prm_keys::Vector)
...
# Arguments
- `true_point`: Vector of true parameter values (e.g. [a, n0])
...
- `w0, m`: fixed model parameters
- `prm_keys`: symbols of inferred parameters, e.g. [:a, :n0]
"""
function plot_ll_grid(point::Vector, prm_keys::Vector, noise_vals::Vector, M_vals::Vector,
                      cutoff::Int64, path_to_read::String; a::Float64=1.3, m::Float64=0.45, n0::Float64=1.0, w0::Float64=1.0)
...
"""
    read_ll_file(w0, n0, a, m, M, noise, path_to_file)

Reads a log-likelihood (LL) file stored in the format: "ll_w0_n0_a_m_M_noise.csv"
...
"""
function read_ll_file(w0, n0, a, m, M, noise, path_to_file::String)
...
"""
    function read_fish_file(w0::Float64, M::Int64, noise::Float64, path_to_file::String)

Read a precomputed Fisher information matrix (FIM) or related data from a CSV file, given the model parameters and noise level.
...
# Fisher information
"""
    function plot_fisher_an0_plane(evals_df::DataFrame, mle::Vector, prm_keys::Vector; logscale::Bool=true)

Plot the Fisher information surface in the (a, n₀) parameter plane as a heatmap with contour lines.

# Arguments
    - `evals_df::DataFrame`: DataFrame containing Fisher information values on a grid
    - `mle::Vector`: Maximum likelihood estimate (MLE) of parameters (not directly plotted here but may guide interpretation)
    - `prm_keys::Vector`: Names of the parameters corresponding to the axes (typically `[:a, :n0]`)
    - `logscale::Bool=true`: If true, plots the logarithm of the Fisher information values (nonpositive values are set to `NaN`)
"""
...
Plot a grid of Fisher information heatmaps across different noise levels and spatial resolutions.

# Arguments
    - `noise_vals::Vector`: Vector of noise levels used in the simulations (increasing top → bottom)
    - `M_vals::Vector`: Vector of spatial resolutions (number of grid points), decreasing left → right
    - `path_to_read::String`: Directory containing the Fisher information CSV files
    - `logscale::Bool=true`: If true, plots the logarithm of Fisher information values (nonpositive values set to `NaN`)
"""
function plot_fisher_grid(noise_vals::Vector, M_vals::Vector,
                          path_to_read::String; logscale::Bool=true)
