...
function compute_sobol_indices(N::Int64, dens_a::Distribution, dens_m::Distribution, dens_w0::Distribution, dens_n0::Distribution,
    ...
    sobol_n = [analyze(data, n[:,j], progress_meter=false, num_resamples=nothing, conf_level=nothing) for j in 1:M]
    sobol_w = [analyze(data, w[:,j], progress_meter=false, num_resamples=nothing, conf_level=nothing) for j in 1:M]
...
function sobol_index_subplot_wn(sobol_n::Vector, sobol_w::Vector; title::String="", M::Int64=1000, t_end::Float64=100.0)
    ...
    # plot n compartment
    plot_n = sobol_index_subplot(sobol_n, "n", title = title, M=M, t_end=t_end)

    # plot w compartment
    plot_w = sobol_index_subplot(sobol_w, "w", M=M, t_end=t_end)
